...
def test_ai_provider_rejects_retired_exo_value(monkeypatch: pytest.MonkeyPatch) -> None:
    """AI_PROVIDER=exo (the retired backend) must fail fast at startup with a
    pydantic ValidationError now that ai_provider is a Literal, instead of
    silently degrading to LM Studio."""
    import pydantic
