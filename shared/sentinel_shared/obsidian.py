"""Shared Obsidian Local REST API client — composable core + mixins (XMOD-01, D-01).

Closes the cross-module duplication of pf2e's ``ObsidianClient``: rather than
combine copies once several modules are running (the anti-pattern this closes
per D-01's "build the core now" guiding principle), the common HTTP plumbing
and universally-shared methods live here once, in ``ObsidianClientCore``, with
the pf2e-only surface split into two composable mixins.

Behavior-preserving lift (D-04): every method body below is copied verbatim
from ``modules/pathfinder/app/obsidian.py`` (pre-cutover) — same 120s
``put_note`` timeout, same depth-8 ``list_directory`` recursion guard, same
content-type handling. Zero behavior drift; this is a structural split, not a
redesign.

Two compositions this package serves:
    # pf2e (Plan 02) needs the full surface:
    class ObsidianClient(ObsidianClientCore, ObsidianBinaryMixin, ObsidianHeadingMixin):
        pass

    # music (Plan 03) needs core-only (MUS-02: never touches binary/heading,
    # and never imports sentinel-core's Vault Protocol):
...
class ObsidianClientCore:
    ...
    Instantiated once per lifespan in a module's main.py and stored on
    app.state. The caller passes a persistent httpx.AsyncClient — do not
    create one here.
    """
...
class ObsidianHeadingMixin:
    """Heading-targeted PATCH — shared: pf2e uses it today; Music/Finance may later."""
...
class ObsidianBinaryMixin:
    """Binary vault I/O — shared but pf2e-only today; composable so no future

    media module has to copy pf2e (currently unused by music, D-01).
    """
