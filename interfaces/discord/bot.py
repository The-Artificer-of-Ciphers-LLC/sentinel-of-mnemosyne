...
def _format_classify_response(data: dict) -> str:
    """Format a /note/classify response dict into a one-line user message."""
...
async def _call_core_inbox_classify(user_id: str, entry_n: int, topic: str) -> str:
    if topic not in _NOTE_CLOSED_VOCAB:
        return f"Unknown topic `{topic}`. Valid: {', '.join(sorted(_NOTE_CLOSED_VOCAB))}"
    payload = {"entry_n": entry_n, "topic": topic}
    try:
        async with httpx.AsyncClient() as http_client:
            data = await _sentinel_client.post_to_module(
                "inbox/classify", payload, http_client
            )
    except Exception as exc:
    ...
    path = data.get("path", "?")
    return (
        f"Filed entry {entry_n} to `{path}` — re-run `:inbox` to see renumbered entries."
    )
...
async def _call_core_inbox_discard(user_id: str, entry_n: int) -> str:
    ...
    try:
        async with httpx.AsyncClient() as http_client:
            data = await _sentinel_client.post_to_module(
                "inbox/discard", payload, http_client
            )
    except Exception as exc:
        logger.warning("inbox discard call failed: %s", exc)
        return f"Inbox discard failed: {exc}"
    return (
        f"Discarded entry {entry_n} — re-run `:inbox` to see renumbered entries."
    )
...
async def _call_core_sweep_status(user_id: str) -> str:
    try:
        async with httpx.AsyncClient() as http_client:
            resp = await http_client.get(
                f"{SENTINEL_CORE_URL.rstrip('/')}/vault/sweep/status",
                headers={"X-Sentinel-Key": SENTINEL_API_KEY},
                timeout=20.0,
            )
            resp.raise_for_status()
