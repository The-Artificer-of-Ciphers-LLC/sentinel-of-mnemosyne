...
## Milestone v0.5.1 — The Second Brain

### Phase 39: Extract the Recall Module

**Goal:** Retrieval becomes a first-class `Recall` module above the Vault seam, returning `RecalledContext`; `MessageProcessor` and `GET /context/{user_id}` both delegate to it. Behavior-preserving extraction — the relevance threshold, namespace exclusions (including `ops/`), and per-tier context budgets move into an explicit `RecallConfig`.
**Depends on:** Phase 38 (builds on shipped v0.5 code; no new phase dependency)
**Requirements:** MEM-01, MEM-02
**Canonical ref:** docs/adr/0003-recall-module.md
**Success Criteria** (what must be TRUE):

  1. Sending a message causes `Recall.assemble()` to be the sole entry point for hot- and warm-tier assembly — `MessageProcessor` contains no inline tier-building logic
  2. `GET /context/{user_id}` returns the same recalled context as the message path (no duplicated assembly logic)
...
**Goal:** A `RetrievalStrategy` seam inside `Recall` with two adapters (`KeywordRecall` wrapping Obsidian BM25 and `SemanticRecall` reading per-note `embedding_b64`); the vault sweeper's per-note embeddings become live retrieval data via a sweeper-maintained `ops/sweeps/embedding-index.json` sidecar; keyword and semantic results are merged via Reciprocal Rank Fusion (k≈60).
**Depends on:** Phase 39
**Requirements:** MEM-03, MEM-04, MEM-05
**Canonical ref:** docs/adr/0004-semantic-recall.md
**Success Criteria** (what must be TRUE):

  1. A query phrased as a paraphrase or near-synonym of a vault note returns that note in `RecalledContext.warm` (keyword-only retrieval would miss it)
  2. `SemanticRecall` reads embeddings from `ops/sweeps/embedding-index.json` — no per-note Obsidian REST call at query time
...
- [x] 40-05-PLAN.md — Vault-seam protected-namespace guard (`PROTECTED_NAMESPACES`/`is_protected_path`/`ProtectedPathError`): enumerated protected set, SOURCE + DESTINATION protection — protected paths can never be relocated/trashed by any caller, nor can content be relocated INTO a protected namespace
...
**Success Criteria** (what must be TRUE):
  ...
  1. The recent-session window is determined by `RetentionPolicy` fields (`hot_limit`, `hot_window_days`) rather than inline magic numbers — changing the policy object changes the window
...
- [x] 48-03-PLAN.md — Wave 2: music module scaffold (FastAPI + registration/heartbeat + healthz + core-only ObsidianClient), zero Core changes (MUS-01, MUS-02)
