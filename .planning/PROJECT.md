...
**Core Infrastructure**
- [x] Pi harness runs in Docker, accepts prompts via RPC (stdin/stdout JSONL), returns structured responses
- [x] Sentinel Core container (FastAPI/Python) receives message envelopes, routes to Pi, returns responses
- [x] LM Studio on Mac Mini confirmed as AI backend via OpenAI-compatible API
- [x] Docker Compose base structure established with override file pattern
- [x] Sentinel can receive a message and return an AI response end-to-end (v0.1)

**Memory Layer**
- [x] Obsidian Local REST API plugin installed and accessible from Core container
- [x] Core retrieves relevant user context from vault before building Pi prompt
- [x] Core writes session summaries to vault after each interaction
- [x] System demonstrates cross-session memory (references a prior conversation) (v0.2)

**Interfaces**
- [x] Standard Message Envelope format defined and stable
- [x] Discord bot interface container operational — sends envelopes, posts responses
- [x] Apple Messages bridge (Mac-side component + HTTP bridge to Core)
- [x] Docker Compose override pattern validated with first real interface (v0.3)

**AI Layer Polish**
- [x] Provider configuration via environment variables (no hardcoding)
- [x] At least two providers testable (LM Studio + one other)
- [x] Error handling, retry logic, and timeout management in Pi client
- [x] Pi harness wrapper API finalized — clean contract rest of system depends on (v0.4)
...
|----------|-----------|---------|
| Pi harness as AI execution layer | Handles conversation loop, tool calls, skill dispatch out of the box — avoids rebuilding this infrastructure | — Implemented |
| Obsidian Local REST API for vault writes | Avoids race conditions of direct file writes; supports surgical edits (PATCH) and full-text search | — Implemented |
| LM Studio as primary AI provider | OpenAI-compatible API makes provider swap frictionless — just change env vars | — Implemented |
| FastAPI (Python) for Sentinel Core | Best ecosystem fit for AI/automation; async handles concurrent interfaces | — Implemented |
| Docker Compose `include` directive for modularity | Base compose never changes; `include` (Compose v2.20+) resolves paths per-file, cleaner than `-f` stacking | — Implemented |
| Pi HTTP bridge inside Pi container | Pi itself is stdin/stdout only — a thin Fastify bridge (~50-100 lines) exposes it over Docker network; not provided by pi-mono | — Implemented |
