...
| **Quick run command** | `cd sentinel-core && python3 -m pytest tests/test_message.py -x -q` |
| **Full suite command** | `cd sentinel-core && python3 -m pytest -x -q` |
...
- **After every task commit:** Run `cd sentinel-core && python3 -m pytest tests/test_message.py -x -q`
- **After every plan wave:** Run `cd sentinel-core && python3 -m pytest -x -q`
...
| 07-02-01 | 02 | 1 | MEM-08 | T-7-02-01 | injection_filter.wrap_context() applied to vault block (untrusted vault content treated as data) | unit | `cd sentinel-core && python3 -m pytest tests/test_message.py -x -k "test_warm_tier_called_on_every_message" -q` | ✅ | ✅ green |
| 07-02-02 | 02 | 1 | MEM-08 | T-7-02-04 | httpx params= dict percent-encodes query (prevents special-char injection into Obsidian search URL) | unit | `cd sentinel-core && python3 -m pytest tests/test_message.py -x -k "warm_tier" -q` | ✅ | ✅ green |
| 07-02-03 | 02 | 2 | MEM-05 | — | N/A | unit | `cd sentinel-core && python3 -m pytest tests/test_message.py -x -q` | ✅ | ✅ green |
| 07-02-04 | 02 | 2 | MEM-08 | T-7-02-01 | wrap_context() applied to search results (same SEC-01 path as hot tier) | unit | `cd sentinel-core && python3 -m pytest tests/test_message.py -x -k "search" -q` | ✅ | ✅ green |
| 07-02-05 | 02 | 2 | MEM-08 | — | N/A | unit | `cd sentinel-core && python3 -m pytest tests/test_obsidian_client.py -x -q` | ✅ | ✅ green |
