...
- **D-05 (reuse match):** Embedding similarity threshold **0.70** within the same topic folder (calibrated Phase 33.1 — see below). On cache hit, the ruling is returned with an italic `_reusing prior ruling on <topic> — confirm applicability_` note in the Discord embed so the DM knows they are reading a prior adjudication rather than a fresh one.

  **D-05 history:** Initially user-locked at 0.80 in pre-execution discuss-phase. Phase 33-05 live UAT (UAT-8) surfaced an empirical mismatch — common paraphrase pairs on `text-embedding-nomic-embed-text-v1.5` cluster in the 0.75-0.79 cosine band, just below the 0.80 ceiling. Operator (2026-04-25) explicitly re-classified D-05 from user-locked to calibrated and authorized a Wave-2-style F1-max sweep on a 40-pair paraphrase fixture (24 paraphrase + 16 same-topic-different-question pairs across 8 anchors). Result: F1-max threshold = **0.70** (F1=0.821, recall=0.958, precision=0.719). Separation is muddy on this embedding model (paraphrase mean 0.787, different mean 0.719, overlap band 0.66-0.81); future model swap to a tighter STS-calibrated embedding (e.g. `bge-large-en-v1.5`) would re-trigger calibration. See Phase 33.1 SUMMARY for the full sweep table.
