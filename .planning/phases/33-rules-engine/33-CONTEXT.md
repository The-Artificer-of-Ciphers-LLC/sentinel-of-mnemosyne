...
### Corpus + ingestion
- **D-01 (seed corpus):** Scan and embed the three Remaster core books — **Player Core**, **GM Core**, **Monster Core** — at module startup. These are ORC-licensed and available via the pf2e Foundry system package (same source family as Phase 32's monster data). The researcher selects the exact ingestion source (Foundry pf2e rules pack vs Archives of Nethys HTML scrape); both are legal under ORC. No other books in this phase.
