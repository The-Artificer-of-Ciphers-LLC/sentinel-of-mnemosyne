...
### Research-surfaced decisions (44-RESEARCH.md, added post-research)
- **D-06 (Retire `_safe_file_chat_note` redirect + fix stale test):** `message.py`'s "guaranteed searchable" redirect becomes structurally unsatisfiable after D-03 (every topic now routes to a warm-excluded destination). Remove the dead redirect; chat-derived notes are episodic (`ops/journal/`, excluded) or staged (`inbox/`) and are recalled via the session/episodic path, not force-routed to a warm location. Fix the existing test that asserts the old guarantee against the stale `_WARM_TIER_EXCLUDE_PREFIXES` list so it checks the real exclusion set and real behavior. (Resolves RESEARCH.md OQ1 + Pitfall 3.)
- **D-07 (Permanent relocation guard for `_`-prefixed inbox control files + test):** Because D-02 removes `inbox/` from `SWEEP_SKIP_PREFIXES`, the `inbox/_pending-classification.md` queue file is newly exposed to the sweeper's topic-move relocation. Add a narrow guard so `_`-prefixed control files are never topic-relocated (embedding of real `inbox/` notes still proceeds), plus a characterizing test. A queue is not a note — relocating it corrupts the queue. (Resolves RESEARCH.md OQ2.)

### Research-surfaced hazards (MUST be handled in the plan — 44-RESEARCH.md)
- **Hardcoded `journal/{today}` literal:** `note_classifier.topic_dir_for()` AND `note_intake._topic_target_path()` hardcode the `journal/` string instead of deriving from `TOPIC_VAULT_PATH["journal"]`. Editing the dict alone has ZERO effect on journal routing — both literals must move to `ops/journal/` together, or the D-03b single-source-of-truth extraction must make the literal derive from the dict.
- **`is_in_topic_dir()` family-root collapse:** `vault_sweep_plan.is_in_topic_dir()` keys on the first path segment only, collapsing `journal`/`accomplishment`/`observation` into one indistinguishable `ops/` family under the new taxonomy — silently breaks the sweeper's misplaced-note relocation for those three. Must be made taxonomy-aware.
- **Regression baseline is 471 tests** (not "404+"): SC-5 gate = `cd sentinel-core && pytest` stays green; 7 named tests need explicit rewrites (not just re-runs), 4 new Wave-0 characterizing tests recommended. See RESEARCH.md for names + line cites.
